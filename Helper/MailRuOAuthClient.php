...
namespace MauticPlugin\MailRuAuthBundle\Helper;

class MailRuOAuthClient
{
    private const TOKEN_URL = 'https://o2.mail.ru/token';

    private const USERINFO_URL = 'https://o2.mail.ru/userinfo';
    ...
    public function exchangeCode(string $code, string $clientId, string $clientSecret, string $redirectUri): array
    {
        ...
        if (empty($data['access_token'])) {
            throw new \RuntimeException('mautic.integration.mailruauth.invalid_token');
        }
    ...
    public function fetchUserInfo(string $accessToken, string $clientId, string $allowedDomain = ''): array
    {
        $data = $this->request(
            'GET',
            self::USERINFO_URL.'?'.http_build_query(['access_token' => $accessToken], '', '&', PHP_QUERY_RFC3986),
            [
                'Accept: application/json',
            ]
        );
        ...
        if ('' === $email || false === filter_var($email, FILTER_VALIDATE_EMAIL)) {
            throw new \RuntimeException('mautic.integration.mailruauth.email_missing');
        }
        ...
        if ('' !== $domain) {
            ...
            if (!hash_equals($domain, $emailDomain)) {
                throw new \RuntimeException('mautic.integration.mailruauth.domain_denied');
            }
        }
    ...
    {
        $headers[] = 'User-Agent: mautic-mailru-auth/1.0';
